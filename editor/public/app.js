...
async function initEditor() {
  try {
    // 1. Cargar el HTML y CSS actual
    const res = await fetch('/api/page');
    const pageResponse = await res.json();

    if (!pageResponse.success || !pageResponse.data) {
      throw new Error(pageResponse.message || 'Error al cargar página');
    }

    const { html, css } = pageResponse.data;
    ...
    try {
      ...
      console.warn('No se pudieron precargar los assets:', e);
    }
    ...
    // 4. Bloques de Memexicanísimos
    const bm = editor.BlockManager;
    const catMemex = { id: 'memexicanisimos', label: '🌮 Memexicanísimos', open: true };
    ...
    bm.add('card-noticia', {
      ...
      content: `
        <article style="background:#141B17; border:1px solid rgba(212,175,55,0.3); border-radius:16px; overflow:hidden; padding:16px; max-width:380px; margin:15px auto; box-shadow:0 8px 24px rgba(0,0,0,0.5);">
          ...
          <div style="font-size:0.75rem; color:#D4AF37; font-weight:700; text-transform:uppercase; margin-bottom:6px; letter-spacing:0.05em;">🌮 Primicia Tracala</div>
          <h3 style="font-size:1.25rem; font-weight:700; color:#F8F9FA; margin-bottom:8px; line-height:1.2;">Nuevo encabezado aquí</h3>
          <p style="font-size:0.9rem; color:#9CA3AF; line-height:1.45; margin-bottom:12px;">Escribe el chisme o noticia patria aquí para que toda la banda se entere.</p>
    ...
    bm.add('card-app-io', {
      ...
      content: `
        <div style="background:rgba(20,27,23,0.85); border:1px solid rgba(212,175,55,0.3); border-radius:16px; padding:20px; max-width:360px; margin:15px auto; box-shadow:0 8px 24px rgba(0,0,0,0.5);">
          <div style="display:flex; align-items:center; gap:12px; margin-bottom:12px;">
            ...
            <div>
              <h3 style="color:#F8F9FA; font-size:1.15rem; font-weight:800; margin:0;">Nombre de la App</h3>
              <span style="font-size:0.75rem; color:#D4AF37; font-family:'Space Mono',monospace;">v1.0 · Open Source</span>
            </div>
          </div>
          <p style="color:#9CA3AF; font-size:0.85rem; line-height:1.4; margin-bottom:16px;">Describe la herramienta y qué problema resuelve para la banda.</p>
          <div style="display:flex; gap:8px;">
            <a href="#" style="flex:1; text-align:center; background:#006847; color:#FFF; padding:8px 12px; border-radius:8px; font-size:0.8rem; font-weight:700; text-decoration:none;">Probar Demo .io</a>
            <a href="#" style="background:rgba(255,255,255,0.08); color:#F8F9FA; padding:8px 12px; border-radius:8px; font-size:0.8rem; font-weight:600; text-decoration:none;">GitHub</a>
          </div>
        </div>
      `
    });

    bm.add('modulo-taco-donacion', {
      label: 'Módulo Taco',
      category: catMemex,
      media: '<i class="fas fa-utensils" style="font-size:20px; color:#F5C542;"></i>',
      content: `
        <div style="background:linear-gradient(135deg, rgba(20,27,23,0.9), rgba(28,38,33,0.95)); border:2px solid rgba(212,175,55,0.4); border-radius:20px; padding:24px; text-align:center; max-width:420px; margin:20px auto; box-shadow:0 12px 32px rgba(0,0,0,0.6);">
          <div style="font-size:2.2rem; margin-bottom:8px;">🌮</div>
          <h3 style="color:#D4AF37; font-size:1.3rem; font-weight:800; margin-bottom:6px;">¿Te laten nuestros proyectos?</h3>
          <p style="color:#9CA3AF; font-size:0.88rem; line-height:1.45; margin-bottom:16px;">Apoya el desarrollo de software libre mexicano invitándole unos taquitos al equipo de Memexicanísimos.</p>
          <a href="#" style="display:inline-flex; align-items:center; gap:8px; background:linear-gradient(135deg, #D4AF37, #B8860B); color:#111; padding:10px 24px; border-radius:999px; font-weight:800; text-decoration:none; box-shadow:0 4px 14px rgba(212,175,55,0.4);">
            <span>🌮</span> ¡Invítame un Taco!
          </a>
        </div>
      `
    });
    ...
    bm.add('banner-alerta-patria', {
      ...
      content: `
        <div style="background:linear-gradient(90deg, rgba(0,104,71,0.25), rgba(206,17,38,0.25)); border-left:4px solid #D4AF37; border-radius:8px; padding:12px 18px; margin:15px auto; max-width:800px; display:flex; align-items:center; gap:12px;">
    ...
    bm.add('tabla-atajos-teclado', {
      ...
      content: `
        <div style="background:#141B17; border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:16px; max-width:440px; margin:15px auto;">
    ...
    bm.add('boton-charro', {
      ...
      content: `
        <a href="#" style="display:inline-flex; align-items:center; gap:8px; background:linear-gradient(135deg,#D4AF37,#B8860B); color:#111; padding:12px 24px; border-radius:999px; font-weight:800; text-decoration:none; box-shadow:0 4px 14px rgba(212,175,55,0.4); margin:8px;">
    ...
    bm.add('caja-glass', {
      ...
      content: `
        <div style="background:rgba(20,27,23,0.75); backdrop-filter:blur(12px); border:1px solid rgba(212,175,55,0.25); border-radius:20px; padding:28px; margin:20px auto; max-width:900px; box-shadow:0 12px 36px rgba(0,0,0,0.6);">
          <h2 style="color:#D4AF37; font-size:1.6rem; font-weight:800; margin-bottom:10px;">Nueva Sección Destacada</h2>
          <p style="color:#F8F9FA; font-size:1rem; line-height:1.5;">Arrastra aquí dentro cualquier bloque, imagen o botón para armar tu diseño.</p>
        </div>
      `
    });
